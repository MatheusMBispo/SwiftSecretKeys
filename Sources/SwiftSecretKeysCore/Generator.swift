...
public struct Generator {
    ...
    private struct ChaChaKeyConfig {
        let name: String
        let combined: [UInt8]  // nonce(12) || ciphertext(n) || tag(16)
    }
    ...
    public func generate(outputDirectory: String? = nil, dryRun: Bool = false) throws {
        ...
        case .chacha20:
            ...
            let keyBytes: [UInt8] = chachaKey.withUnsafeBytes { Array($0) }

            var chachaKeyConfigs = [ChaChaKeyConfig]()
            for (key, value) in config.keys {
                let sanitizedName = nameMap[key]!
                let combined = try encryptChaCha20(value, using: chachaKey)
                chachaKeyConfigs.append(ChaChaKeyConfig(name: sanitizedName, combined: combined))
            }
            chachaKeyConfigs.sort { $0.name < $1.name }

            rendered = renderChaCha20Output(keyBytes: keyBytes, keyConfigs: chachaKeyConfigs)
        }
    ...
    private func encryptChaCha20(_ value: String, using key: SymmetricKey) throws -> [UInt8] {
        let nonce = ChaChaPoly.Nonce()  // Fresh 12-byte random nonce per value
        let sealedBox = try ChaChaPoly.seal(Data(value.utf8), using: key, nonce: nonce)
        return Array(sealedBox.combined)  // ChaChaPoly.SealedBox.combined is Data (non-optional)
    }
    ...
    private func renderOutput(saltChunks: [[UInt8]], keyConfigs: [KeyConfig]) -> String {
        ...
        return """
        // Generated by SwiftSecretKeys \(SwiftSecretKeysVersion.current) — do not edit.
    ...
    private func renderChaCha20Output(keyBytes: [UInt8], keyConfigs: [ChaChaKeyConfig]) -> String {
        let formatter = ISO8601DateFormatter()
        formatter.timeZone = TimeZone(identifier: "UTC")
        let timestamp = formatter.string(from: Date())
        ...
        let varBlocks = keyConfigs.map { keyConfig in
            ...
            \(combinedLines)
                    ...
                    return SecretKeys._decryptChaCha20(combined)
                }
            """
        }.joined(separator: "\n\n")
        ...
        // SECURITY NOTICE
        // ===============
        // This file uses ChaCha20-Poly1305 obfuscation, not a secrets vault.
        // The ChaCha20 key and nonce are embedded in this file alongside the ciphertext.
        // A determined attacker with access to the compiled binary can recover
        // the original values.
        //
        // OWASP MASVS-RESILIENCE context: this technique provides
        ...
        enum SecretKeys {
            private static let chachaKey: [UInt8] = [
        \(keyLines)
            ]

        \(varBlocks)

            private static func _decryptChaCha20(_ combined: [UInt8]) -> String {
                let key = SymmetricKey(data: Data(chachaKey))
                guard let sealedBox = try? ChaChaPoly.SealedBox(combined: Data(combined)) else {
                    preconditionFailure("SwiftSecretKeys: ChaCha20 SealedBox initialization failed — corrupted generated data")
                }
                guard let plaintext = try? ChaChaPoly.open(sealedBox, using: key) else {
                    preconditionFailure("SwiftSecretKeys: ChaCha20 decryption failed — key or data corrupted")
                }
                return String(decoding: plaintext, as: UTF8.self)
            }
        }
