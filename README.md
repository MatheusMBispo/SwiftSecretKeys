...
Since SPM plugins cannot accept custom arguments, additional CLI flags are controlled via environment variables:

| Variable | CLI Equivalent | Description |
|---|---|---|
| `SSKEYS_ENVIRONMENT` | `--environment` | Target environment name (`dev`, `staging`, `prod`) |
| `SSKEYS_FACTOR` | `--factor` | Custom salt length in bytes for XOR mode (default: 32) |
| `SSKEYS_ENV_FILE` | `--env-file` | Path to a `.env` file to load before generation |
| `SSKEYS_VERBOSE` | `--verbose` | Set to any value to enable verbose output to stderr |

```bash
# Example: CI build with custom factor and .env file
SSKEYS_ENVIRONMENT=prod SSKEYS_FACTOR=64 SSKEYS_ENV_FILE=.env.prod swift build
```
